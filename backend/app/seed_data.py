# Seed data generation for CDP with 1000+ orders and Super Admin
# Run this script to populate the database with demo data for development
...
from app.models import Customer, Order, OrderItem, Product, Insight, Segment, SegmentRule, Flow, FlowStep, User, UserRole
from app import auth


# ============== SUPER ADMIN CREDENTIALS ==============
SUPER_ADMIN_EMAIL = "admin@hyperverge.co"
...
def seed_database():
    ...
    try:
        print_separator()
        print("🚀 HYPERVERGE CDP - DATABASE SEEDING")
        print_separator()
        ...
        if not admin:
            superuser = User(
                ...
                hashed_password=auth.get_password_hash(SUPER_ADMIN_PASSWORD),
        ...
        # ============== CHECK IF CDP DATA EXISTS ==============
        if db.query(Customer).count() > 0:
            print("\n⏭️  CDP data already exists, skipping data seeding...")
            print_separator()
            print("\n🎉 DATABASE READY!")
            print_separator()
            print("\n📋 SUPER ADMIN CREDENTIALS:")
        ...
        # ============== CREATE SEGMENTS ==============
        print("   🎯 Creating 5 sample segments...")
        segments_data = [
            {
        ...
        db.commit()
        print("   ✅ 5 segments created")
        
        # ============== CREATE FLOWS ==============
        print("   📧 Creating 3 email marketing flows...")
        flows_data = [
            {
                "name": "Welcome Series",
        ...
        db.commit()
        print("   ✅ 3 flows created (with 9 email steps)")
        
        # ============== CREATE INSIGHTS ==============
        print("   💡 Creating 6 dashboard insights...")
        ...
        print_separator()
        print("\n📊 DATA SUMMARY:")
        print(f"   • Super Admin:  1 user")
        print(f"   • Products:     20 items")
        print(f"   • Customers:    150 records")
        print(f"   • Orders:       {order_count} transactions")
        print(f"   • Segments:     5 customer segments")
        print(f"   • Flows:        3 email flows (9 steps)")
        print(f"   • Insights:     6 dashboard insights")
        print_separator()
        print("\n📋 SUPER ADMIN CREDENTIALS:")
        print(f"   Email:    {SUPER_ADMIN_EMAIL}")
        print(f"   Password: {SUPER_ADMIN_PASSWORD}")
        print_separator()
        print("\n✨ Your CDP platform is ready to use!")
        print_separator()
